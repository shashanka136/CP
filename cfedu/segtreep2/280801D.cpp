...
ll a;
template<class node, class node1, int typ>
struct segtree {
    int n;
    vector<node> st;
    vector<node1> lazy;
    ...
    void push(node1 &x, int l, int r, int p) {
        if(!(x == node1())) {
            if(typ == 1) st[p].n = st[p].n + x.n *(r-l+1);
            else st[p].n += x.n *l *(r-l+1) + x.n * (r-l+1)*(r-l+2)/2;
            if(l != r) {
...
typedef segtree<item, item_lazy, 0> segm1;
typedef segtree<item, item_lazy, 1> segm2;
...
void build(int l, int r, int p) {
    ...
    int m = l + r >> 1;
    build(l, m, fo1.le(p));
    build(m + 1, r, fo1.ri(p));
    fo1.st[p] = fo1.st[fo1.le(p)] + fo1.st[fo1.ri(p)];
    fo2.st[p] = fo2.st[fo2.le(p)] + fo2.st[fo2.ri(p)];
}
void solve(){
	cin>>n>>m;
	fo1.init(n);
	...
	for(int i =0; i<m; i++){
		...
		if(typ == 1){
			...
			cin>>l>>r;
			l--,r--;
			cout<<fo1.query(l,r).n - fo2.query(l,r).n * l<<endl;
		}
	}
}
