...
struct segtree {
    ...
    void push(node1 &x, int l, int r, int p) {
        if(!(x == node1())) {
            st[p].sm = x.n*(r-l+1);
            st[p].num = st[p].suf = st[p].pre = x.n;
            if(l != r) {
    ...
    template<int x>
    void add(int ql, int qr, int l, int r, int p) {
        push(lazy[p],l,r,p);
        if (qr < l || ql > r) return;
        if (ql <=l && r <= qr) {
            lazy[p] = x;
        ...
        int m = l + r >> 1;
        add<x>(ql, qr, l, m, le(p));
        add<x>(ql, qr, m + 1, r, ri(p));
...
void solve() {
	...
	for(int i = 0; i<n; i++) {
        ...
        cin>>x>>l;
		if(c == 'W') {
            fo.add<0>(N+x,N+x+l-1);
		}else {
            fo.add<1>(N+x,N+x+l-1);
		}
        fo.push(fo.lazy[0], 0, fo.n-1, 0);
        cout<<fo.st[0].num<<" "<<fo.st[0].sm<<endl;
	}
}
