...
bool cmp(const int &i, const int &j){
	return (fo.lcp[i] > fo.lcp[j]) || (fo.lcp[i] == fo.lcp[j] && i < j);
}
int fin(int i){
	if(p[i] != i){
		p[i] = fin(p[i]);
...
void solve(){
	...
	iota(ind,ind+cr,0);
	iota(p,p+cr,0);
	sort(ind,ind+cr-1,cmp);
	fill(siz,siz+cr,1);
	for(int i =0; i<cr-1; i++){
		un(ind[i],ind[i]+1);
		ans = max(ans,1ll*(fo.lcp[ind[i]]) * siz[fin(ind[i])]);
	}
	cout<<ans<<endl;
}
